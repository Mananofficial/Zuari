...
const Homepage = () => {
  ...
  const rows = [
    {
      ...
      bullets: ["24/7 doctor on chat", "Family enrollment in 3 taps", "Wellness credits & rewards"],
      img: caringImg,
      align: "left",
    },
    {
      ...
      bullets: ["Parents cover up to 80 yrs", "Pan-India cashless network", "Maternity & newborn included"],
      img: family,
      align: "right",
    },
    {
      ...
      bullets: ["Bulk CSV onboarding", "Live claims analytics", "Slack & HRMS integrations"],
      img: hero,
      align: "left",
    },
  ];
  ...
  const insuranceitems = [
    { q: "Onboarding 300 employees took a weekend. Claims that used to take weeks now settle in a day.", n: "Priya Menon", r: "Head of People, Northwind" },
    { q: "The dashboard alone saved our HR team 20 hours a month. But the humans behind it are the real product.", n: "Rahul Iyer", r: "COO, Vertex Labs" },
    ...
    <div>
      ...
      <section id="solutions" className="mx-auto max-w-7xl px-6 py-24">
        <div className="mb-12 flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
          <div>
            <span className="text-sm font-semibold uppercase tracking-wider text-(--coral)">What we do</span>
            <h2 className="mt-2 max-w-2xl font-display text-4xl md:text-5xl">
              Insurance programs, <span className="italic">designed</span> — not just sold.
            </h2>
          </div>
          <p className="max-w-md text-(--muted-foreground)">
            Whether you're a 20-person startup or a 5,000-person enterprise, we shape a program around your people, industry, and budget.
          </p>
        </div>
          ...
          {items.map((it, i) => {
            ...
            return (
              ...
              >
                ...
                <span className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl ${it.color}`}>
      ...
      <section className="mx-auto max-w-7xl space-y-24 px-6 py-24">
        {rows.map(({ badge, title, body, bullets, img, align }, idx) => (

          <div
            key={idx}
            ref={ref}
            className={`grid gap-12 md:grid-cols-2 md:items-center ${align === "right" ? "md:[&>*:first-child]:order-2" : ""}`}
            style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(40px)", transition: "all 0.9s ease-out" }}
          >
            <div className="relative">
              <div className="absolute -inset-4 -z-10 rounded-[2.5rem] bg-grad-cool opacity-20 blur-2xl animate-blob" />
              <div className="overflow-hidden rounded-[2rem] bg-(--card) shadow-soft">
                <img src={img} alt={title} loading="lazy" width={1000} height={1200} className="h-full w-full object-cover transition-transform duration-700 hover:scale-105" />
              </div>
            </div>
            <div>
              ...
              <h3 className="mt-4 font-display text-4xl leading-tight md:text-5xl">{title}</h3>
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-center gap-6 px-6 text-center text-white">
        ...
        <h2 className="max-w-3xl font-display text-4xl md:text-6xl">
          A human on the other end. <span className="italic">Always.</span>
        </h2>
        <p className="max-w-2xl text-white/90">
          When a claim happens, a dedicated Zuari Insurance Brokers Limited specialist takes over — coordinating with the hospital, chasing paperwork, and keeping your team informed every step of the way.
        </p>
        <Link
          to="/contact"
    ...
    <section className="mx-auto max-w-7xl px-6 py-24">
      <div className="mb-12 flex items-end justify-between gap-6">
        <h2 className="max-w-2xl font-display text-4xl md:text-5xl">
          Loved by the teams that <span className="text-grad">wear the pager</span>.
        </h2>
        <div className="hidden items-center gap-1 md:flex">
          {[1,2,3,4,5].map((i) => <Star key={i} className="h-5 w-5 fill-(--coral) text-(--coral)" />)}
          <span className="ml-2 text-sm font-semibold">4.9 · 800+ reviews</span>
        </div>
      </div>
      <div className="grid gap-6 md:grid-cols-3">
        {insuranceitems.map((t, i) => (
          <figure
            key={t.n}
          ...
          >
            ...
            <div className="mb-4 flex gap-1">
              {[1,2,3,4,5].map((s) => <Star key={s} className="h-4 w-4 fill-(--coral) text-(--coral)" />)}
            </div>
        ...
        <div className="relative grid gap-8 md:grid-cols-2 md:items-center">
          <div>
            <h2 className="font-display text-4xl leading-tight md:text-6xl">
              Ready for insurance that <span className="italic">works</span>?
            </h2>
            <p className="mt-4 text-(--primary-foreground)/80">Book a 20-minute call. We'll design a program tailored to your team — no obligations.</p>
          </div>
          <div className="flex flex-col gap-3 md:items-end">
            <Link to="/contact" className="group inline-flex items-center gap-2 rounded-full bg-white px-7 py-4 text-base font-semibold text-(--foreground) shadow-soft transition hover:-translate-y-0.5">
              Get a quote
              <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" />
            </Link>
            <Link to="/solutions" className="inline-flex items-center gap-2 rounded-full border border-white/30 px-6 py-3 text-sm font-medium text-(--primary-foreground)/90 transition hover:bg-white/10">
              Explore solutions
            </Link>
          </div>
        </div>
